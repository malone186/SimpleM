...
$apiUrl = gcloud run services describe brewnote-api --region $region --format "value(status.url)"
if (-not $apiUrl) { throw "brewnote-api 서비스를 찾을 수 없습니다 — deploy_backend.ps1을 먼저 실행하세요." }
...
gcloud run deploy brewnote-web `
    --source "$root\frontend" `
    --region $region `
    --allow-unauthenticated `
    --memory 512Mi

$webUrl = gcloud run services describe brewnote-web --region $region --format "value(status.url)"
