...
class OperationService:
    ...
    # 주의: 아래 3개는 실제 데이터가 아니라 데모용 예시 문구다. 챗봇이 이 수치를 사장님께
    # 진짜처럼 전달하면 안 되므로, 반환 문자열 맨 앞에 예시임을 명시한다(라벨이 숫자와 함께
    # 따라가야 모델이 '예시'로 전한다). 실제 요약은 경영 리포트·판매 예측 도구가 낸다.
    _DEMO_PREFIX = "[예시 데이터 · 실제 매장 수치가 아닙니다] "

    @staticmethod
    def get_daily_operation_summary() -> str:
        """데모용 일간 운영 요약 예시 문구 (실데이터 아님)."""
        return OperationService._DEMO_PREFIX + "오늘 매장 운영 요약: 총 3건의 스케줄이 계획되어 있으며, 바리스타 2명과 매니저 1명이 교대 근무합니다. 예상 일매출은 단순 평균 500,000원 선으로 전망됩니다."

    @staticmethod
    def get_weekly_operation_summary() -> str:
        """데모용 주간 운영 요약 예시 문구 (실데이터 아님)."""
        return OperationService._DEMO_PREFIX + "이번 주 매출은 지난주보다 8% 감소했으나, 파트타이머 근무 가중 적용으로 인해 인건비 지출은 지난주보다 3% 증가했습니다. 예상 세금 관리는 전주 대비 변동이 없습니다."

    @staticmethod
    def get_monthly_operation_summary() -> str:
        """데모용 월간 운영 요약 예시 문구 (실데이터 아님)."""
        return OperationService._DEMO_PREFIX + "이번 달(2026-07) 매장 운영 요약: 가상 매출 총합 3,150,000원과 지출 총합 150,000원, 인건비 합산액을 공제한 예상 순이익은 약 2,000,000원으로 시뮬레이션되었습니다. (참고용 계산)"
