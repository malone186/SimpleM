...
- 영수증에서 품목명 아래 줄에 홀로 적힌 긴 바코드 숫자(예: 8809599360081)는 품목이 아니므로 무시하세요. 수량·금액이 품목명과 다른 줄에 있어도 같은 품목으로 묶으세요. 단, 품목명 자리에 바코드 숫자가 적혀 있고 그 행에 단가·수량·금액이 있으면 그것은 실제 품목입니다 — 숫자 이름 그대로 두고 값을 채우세요.
- "행사", "할인" 같은 표시는 품목이 아닙니다. 과세물품가액=subtotal, 부가세=tax, 합계=total로 취급하세요.
- 품목 표의 열 순서는 영수증마다 다릅니다 (단가·수량·금액 순도 있고 수량·단가·금액 순도 있음). 표 머리글(단가, 수량, @, 금액 등)을 먼저 확인해 각 숫자를 올바른 필드에 넣고, 수량×단가=금액 관계로 교차 검산하세요. 보통 수량은 작은 정수, 단가·금액은 큰 수입니다.
- 도트 프린터·저화질 글자도 최대한 읽으세요. 금액 숫자는 자릿수와 합계 관계(품목 합=소계, 공급가액+세액=합계)로 검산해 읽으세요.
