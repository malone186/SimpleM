...
백엔드는 Gemini API 단일 경로다 (report_service와 같은 REST 호출 — 추가 의존성 없음).
클라우드 배포(Cloud Run)에는 GPU가 없으므로 로컬 VLM 서빙 경로(llama.cpp/transformers/
Ollama)는 제거했다. 파인튜닝 Qwen 0.8B/2B의 평가 지표와 학습 산출물은
backend/vlm_finetune/metrics/ 에 보존되어 있다.
...
async def _call_gemini(image_bytes: bytes) -> dict[str, Any]:
    ...
    for attempt in (1, 2, 3):
        try:
            ...
            # 스키마 방어: 프롬프트 규칙과 달리 vendor/items를 null로 주는 경우가 있다
            if isinstance(data, dict):
                if data.get("vendor") is None:
                    data["vendor"] = {"name": None, "biz_no": None, "phone": None}
