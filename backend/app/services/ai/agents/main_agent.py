...
# 무료 티어 일일 쿼터(RPD)는 모델별로 따로 계산된다 — 주 모델이 429(쿼터 소진)를 맞으면
# 이 예비 모델로 전환해 같은 턴을 재시도한다. 주 모델과 같으면 폴백은 비활성.
...
# 주 모델 쿼터 소진(429) 시 이 시각까지는 처음부터 예비 모델로 실행한다
# — 매 턴 주 모델에 재도전하며 사용자를 수십 초씩 기다리게 하지 않기 위함
_quota_fallback_until = 0.0
_QUOTA_FALLBACK_SECONDS = 600.0
...
async def generate_response(
    ...
    # 주 모델이 최근에 쿼터 소진됐으면 처음부터 예비 모델로 실행
    can_fallback = bool(GEMINI_FALLBACK_MODEL) and GEMINI_FALLBACK_MODEL != GEMINI_MODEL
    active_model = GEMINI_MODEL
    if can_fallback and time.time() < _quota_fallback_until:
        active_model = GEMINI_FALLBACK_MODEL

    try:
        try:
            answer = await _run_turn(active_model)
        except Exception as e:
            # 무료 티어 일일 쿼터는 모델별 — 주 모델 소진이면 예비 모델로 같은 턴을 재시도
            if _is_quota_error(e) and can_fallback and active_model == GEMINI_MODEL:
                _quota_fallback_until = time.time() + _QUOTA_FALLBACK_SECONDS
                logger.warning("주 모델 %s 쿼터 소진(429) — %s(으)로 전환해 재시도, %.0f초간 유지",
                               GEMINI_MODEL, GEMINI_FALLBACK_MODEL, _QUOTA_FALLBACK_SECONDS)
                answer = await _run_turn(GEMINI_FALLBACK_MODEL)
            else:
                raise
        ...
        # 예비 모델까지 쿼터 소진 — 원인을 정확히 알려줘야 사용자가 상황을 이해한다
