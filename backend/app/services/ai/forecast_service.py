...
워크플로우:
  GPS(매장 위치) → 지역 날씨(기상청 단기·중기예보, KMA_SERVICE_KEY 필요) + 요일·공휴일
  + 주변 행사(수동 입력/부스팅) + POS 판매 데이터(Sale) → 시계열 모델로 기본 예측
  → 날씨·행사 보정 → 익일·금주 예상 판매량 → 레시피 기반 재료 소요량 → 발주 추천
...
_weather_cache: dict[str, tuple[float, dict]] = {}  # 기상청 예보 캐시 (30분) — 무료 쿼터 절약
...
# 기상청 하늘상태(SKY) 코드 → 한글 날씨 (강수 없음일 때)
_SKY_KO = {1: "맑음", 3: "구름", 4: "흐림"}
...
def _pty_label(pty: int) -> Optional[str]:
    """기상청 강수형태(PTY) 코드 → 한글 날씨. 강수 없음(0)이면 None."""
    if pty in (1, 2, 5, 6):
        return "비"
    if pty in (3, 7):
        return "눈"
...
def _wf_label(wf: str) -> str:
    """기상청 중기예보 하늘상태 문구('흐리고 비' 등) → 단기예보와 같은 한글 라벨."""
    if "소나기" in wf:
        return "소나기"
    if "비" in wf:
        return "비"
...
# ---------------------------------------------------------------------------
# 3) 날씨 — 기상청(KMA) 단기예보 + 중기예보 (공공데이터포털 KMA_SERVICE_KEY 필요)
# ---------------------------------------------------------------------------

def _latlon_to_kma_grid(lat: float, lon: float) -> tuple[int, int]:
    """위경도 → 기상청 단기예보 격자(nx, ny). 기상청 공식 LCC(람베르트 정각원추) 변환."""
    ...
    re = RE / GRID
    slat1, slat2 = SLAT1 * rad, SLAT2 * rad
    olon, olat = OLON * rad, OLAT * rad
...
# 중기예보 발표구역 — 매장에서 가장 가까운 대표 도시의 (기온 regId, 육상 regId)를 쓴다
_MID_REGIONS = [
    (37.566, 126.978, "11B10101", "11B00000"),  # 서울
    (37.456, 126.705, "11B20201", "11B00000"),  # 인천
    (37.263, 127.028, "11B20601", "11B00000"),  # 수원
    (37.881, 127.730, "11D10301", "11D10000"),  # 춘천
    (37.751, 128.876, "11D20501", "11D20000"),  # 강릉
    (36.642, 127.489, "11C10301", "11C10000"),  # 청주
    (36.350, 127.385, "11C20401", "11C20000"),  # 대전
    (35.824, 127.148, "11F10201", "11F10000"),  # 전주
    (35.160, 126.851, "11F20501", "11F20000"),  # 광주
    (35.871, 128.601, "11H10701", "11H10000"),  # 대구
    (35.180, 129.075, "11H20201", "11H20000"),  # 부산
    (35.539, 129.311, "11H20101", "11H20000"),  # 울산
    (35.228, 128.681, "11H20301", "11H20000"),  # 창원
    (33.500, 126.531, "11G00201", "11G00000"),  # 제주
]


def _kma_service_key() -> str:
    """공공데이터포털 키. URL 인코딩된 키를 넣어도 되도록 디코딩해서 돌려준다."""
    from urllib.parse import unquote

    return unquote(os.getenv("KMA_SERVICE_KEY", "").strip())


def _fetch_weather_short(lat: float, lon: float, key: str) -> dict[str, dict[str, Any]]:
    """기상청 단기예보(getVilageFcst) — 오늘~+3일의 일별 날씨·최고기온·강수확률."""
    import requests

    now = datetime.now(KST)
    ref = now - timedelta(minutes=30)  # 발표(02·05·…·23시) 후 제공까지 ~10분 지연 감안
    base = None
    for h in (23, 20, 17, 14, 11, 8, 5, 2):
        if ref.hour >= h:
            base = ref.replace(hour=h)
            break
    if base is None:
        base = (ref - timedelta(days=1)).replace(hour=23)

    nx, ny = _latlon_to_kma_grid(lat, lon)
    r = requests.get(
        "http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst",
        params={"serviceKey": key, "dataType": "JSON", "numOfRows": 1000, "pageNo": 1,
                "base_date": base.strftime("%Y%m%d"), "base_time": f"{base.hour:02d}00",
                "nx": nx, "ny": ny},
        timeout=8,
    )
    r.raise_for_status()
    ...
    # 일자별로 POP 최대·TMX·시간별 TMP/SKY/PTY를 모은다
    by_day: dict[str, dict[str, Any]] = {}
    for it in items:
        d = f"{it['fcstDate'][:4]}-{it['fcstDate'][4:6]}-{it['fcstDate'][6:]}"
        day = by_day.setdefault(d, {"pop": 0, "tmx": None, "tmp": [], "sky": [], "pty": []})
        cat, val = it["category"], it["fcstValue"]
        try:
            if cat == "POP":
                day["pop"] = max(day["pop"], int(float(val)))
            elif cat == "TMX":
                day["tmx"] = float(val)
            elif cat == "TMP":
                day["tmp"].append(float(val))
            elif cat == "SKY":
                day["sky"].append(int(float(val)))
            elif cat == "PTY":
                day["pty"].append(int(float(val)))
    ...
    for d, v in by_day.items():
        cond = None
        for pty in v["pty"]:  # 하루 중 강수형태가 있으면 우선 (비>눈>소나기 순 아님 — 첫 강수 유형)
            cond = _pty_label(pty)
            if cond:
                break
        if cond is None:
            cond = _SKY_KO.get(max(v["sky"]) if v["sky"] else 4, "흐림")
        temp_max = v["tmx"] if v["tmx"] is not None else (max(v["tmp"]) if v["tmp"] else None)
        out[d] = {"condition": cond, "temp_max": temp_max, "precip_prob": v["pop"]}
    return out


def _fetch_weather_mid(lat: float, lon: float, need_dates: list[str], key: str) -> dict[str, dict[str, Any]]:
    """기상청 중기예보(getMidLandFcst + getMidTa) — 발표 후 4~10일의 일별 날씨."""
    import requests

    now = datetime.now(KST)
    if now.hour >= 18:
        tm_fc, base_day = now.strftime("%Y%m%d") + "1800", now.date()
    elif now.hour >= 6:
        tm_fc, base_day = now.strftime("%Y%m%d") + "0600", now.date()
    else:
        y = now - timedelta(days=1)
        tm_fc, base_day = y.strftime("%Y%m%d") + "1800", y.date()

    ta_reg, land_reg = min(
        _MID_REGIONS, key=lambda r: _haversine_km(lat, lon, r[0], r[1]))[2:4]

    def _first_item(endpoint: str, reg_id: str) -> dict:
        r = requests.get(
    ...
    land = _first_item("getMidLandFcst", land_reg)
    ta = _first_item("getMidTa", ta_reg)

    out: dict[str, dict[str, Any]] = {}
    for iso in need_dates:
        n = (date.fromisoformat(iso) - base_day).days
        if not 4 <= n <= 10:
            continue
        if n <= 7:  # 4~7일 후는 오전/오후 분리 발표 — 강수확률은 최대, 날씨는 오후 우선
            pop = max(int(land.get(f"rnSt{n}Am") or 0), int(land.get(f"rnSt{n}Pm") or 0))
            wf = land.get(f"wf{n}Pm") or land.get(f"wf{n}Am") or ""
        else:
            pop = int(land.get(f"rnSt{n}") or 0)
            wf = land.get(f"wf{n}") or ""
        tmax = ta.get(f"taMax{n}")
        out[iso] = {
            "condition": _wf_label(str(wf)),
            "temp_max": float(tmax) if tmax not in (None, "") else None,
            "precip_prob": pop,
        }
    return out
...
def _fetch_weather(lat: float, lon: float, days: int) -> dict[str, dict[str, Any]]:
    """일자별 날씨 예보 — 기상청 단기예보(+3일) + 중기예보(4~10일).

    KMA_SERVICE_KEY(공공데이터포털, 무료 즉시 발급)가 없거나 실패하면
    날씨 보정 없이 예측을 계속한다.
    """
    key = _kma_service_key()
    if not key:
        logger.warning("KMA_SERVICE_KEY 미설정 — 날씨 보정 없이 예측 (data.go.kr에서 "
                       "'기상청_단기예보'·'기상청_중기예보' 활용신청 후 .env에 키 추가)")
        return {}
    ...
    try:
        weather.update(_fetch_weather_short(lat, lon, key))
    except Exception:
        logger.warning("기상청 단기예보 조회 실패 — 해당 구간 날씨 보정 생략", exc_info=True)

    # 단기예보가 못 덮는 날짜(내일부터 days일 중)는 중기예보로 채운다
    start = date.today() + timedelta(days=1)
    need = [d.isoformat() for d in (start + timedelta(days=i) for i in range(days))
            if d.isoformat() not in weather]
    if need:
        try:
            weather.update(_fetch_weather_mid(lat, lon, need, key))
        except Exception:
            logger.warning("기상청 중기예보 조회 실패 — 해당 구간 날씨 보정 생략", exc_info=True)
