...
# [직원 계정 접근 범위] 직원 토큰은 허용 목록(단골·재고·인증) 밖의 경로를 부를 수 없다.
#  - 엔드포인트마다 막는 대신 여기 한 곳에서 뒤집어 막는다. '막을 것을 세는' 방식은
#    반드시 빠뜨리고, 앞으로 추가되는 엔드포인트도 기본이 '열림'이라 또 뚫린다.
#  - 실제로 직원 토큰으로 찔러보니 재고·메뉴 원가·지출이 그대로 뚫려 있었다.
